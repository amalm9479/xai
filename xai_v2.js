...
rl.question('Choose an option:\n1. Start from the beginning\n2. Start from a specific index\nEnter your choice: ', async (answer) => {
  if (answer === '2') {
    rl.question(`Enter the index to start from (0 to ${addresses.reverse().length - 1}): `, (index) => {
      const inputIndex = parseInt(index, 10);
      if (!isNaN(inputIndex) && inputIndex >= 0 && inputIndex < addresses.reverse().length) {
...
while (true) { // Infinite loop
  const address = addresses[i % addresses.reverse().length]; // Cycle through the addresses
  ...
  // Navigate to the site only for the first address in the first iteration
  if (i === startIndex) {
    await driver.get('https://vanguard.xai.games/');
    console.log('Navigated to https://vanguard.xai.games/');
  }
  ...
  // Wait for 3 seconds after all buttons are claimed before closing the browser
