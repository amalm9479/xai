...
// Declare a global variable to keep track of total successful joins
let totalSuccessfulJoins = 0;
  ...
  try {
    ...
    while (true) {
      try {
        ...
        if (Date.now() - lastRefreshTime > 20 * 60 * 1000) { // 20 minutes in milliseconds
        ...
        for (const joinDrawButton of joinDrawButtons) {
          try {
            ...
            if (prizeAmount > 0 || attemptsAmount > 0) {
              ...
              // Increment and log the total successful joins
              totalSuccessfulJoins++;
              console.log(chalk.blue(`Total Successful Joins: ${totalSuccessfulJoins}`));
            }
          } catch (clickError) {
